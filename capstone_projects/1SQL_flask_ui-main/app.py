...
def index():
    ...
    top_category2 = pd.crosstab(index= df2['Category'],columns='Jumlah').sort_values(by='Jumlah', ascending=False)
    ...
    stats = {
        ...
        'rev_table' : df2.groupby(['Category','App'])[["Reviews","Rating"]].mean().sort_values(by='Reviews', ascending=False).reset_index().head(10).to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    # variabel result akan dimasukkan ke dalam parameter di fungsi render_template() agar dapat ditampilkan di halaman html
    ...
    plt.ylabel('Rating')
    plt.savefig('newPlot.png',bbox_inches="tight")
    ...
    # ----------------------------------------------------------------#
    # why the canvas is cut off? the plot cannot be displayed in full
